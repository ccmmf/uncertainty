...
partition_variance_sources <- function(sobol_indices,
  ...
  required_var_cols <- c("runid", "variable", "ensemble_variance")
  ...
  var_cat <- sobol_joined %>%
    dplyr::group_by(runid, variable, category) %>%
    dplyr::summarize(
  ...
  var_cat <- var_cat %>%
    dplyr::left_join(
      sobol_joined %>%
        dplyr::distinct(runid, variable, Var_total),
      by = c("runid", "variable")
    )

  # compute interaction term per runid * variable
  var_cat <- var_cat %>%
    dplyr::group_by(runid, variable) %>%
    dplyr::mutate(
  ...
  interaction_rows <- var_cat %>%
    dplyr::distinct(runid, variable, Var_total, Var_interaction) %>%
    dplyr::mutate(
  ...
  variance_partition_site <- var_cat %>%
    dplyr::select(runid, variable, category,
